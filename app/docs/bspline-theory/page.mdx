...
As a result, BSplines do not pass through any of the specified control points, as only the endpoints are interpolated.
However, we devised a way to get around this by applying clever mathematical concepts.
...
that represent the $T$ value in the polynomial. The $C$ matrix, or Characteristic matrix is the thing that defines the
...
Now, let's input the values into the $C$ matrix and assemble it all together!
